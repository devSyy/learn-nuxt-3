  ...
  <div class="q-pa-xl">
    <AppCard>
      ...
      <p class="q-mt-lg text-grey-8">{{ course?.content }}</p>
          ...
          <q-btn stretch flat label="Youtube" no-caps @click="moveYoutube"></q-btn>
          <q-separator class="q-mb-lg" />
    <q-form class="q-gutter-y-md">
      ...
      <template #footer>
        <q-btn v-if="prevCourse" label="이전 강의" color="primary" @click="movePage(prevCourse.path)" unelevated :to="prevCourse.path"></q-btn>
        <q-btn label="쿼리추가" color="dark" unelevated :to="{ path: $route.path, query: {timestamp: Date.now()}}"></q-btn>
        <q-space></q-space>
        <q-btn v-if="nextCourse" label="다음 강의" color="primary" @click="movePage(nextCourse.path)" unelevated :to="nextCourse.path"></q-btn>
      </template>
    </AppCard>
  </div>
</template>
